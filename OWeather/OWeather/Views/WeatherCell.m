...
//@property (nonatomic, strong) UIStackView *wrapStack;
...
- (void)setup {
    ...
    self.iconView.frame = CGRectMake(0, 0, 10, 10);
...
+ (UIImage *)weatherIcon:(NSString *)descriptor {
    if ([descriptor isEqual:@"01d"]) {
        return [UIImage systemImageNamed: @"sun"];
    } else if ([descriptor isEqual:@"02d"]) {
        return [UIImage systemImageNamed:@"cloud.sun"];
    } else if ([descriptor isEqual:@"03d"] || [descriptor isEqual:@"04d"]) {
        return [UIImage systemImageNamed:@"cloud"];
    } else if ([descriptor isEqual:@"09d"]) {
        return [UIImage systemImageNamed:@"cloud.rain"];
    } else if ([descriptor isEqual:@"10d"]) {
        return [UIImage systemImageNamed:@"cloud.sun.rain"];
    } else if ([descriptor isEqual:@"11d"]) {
        return [UIImage systemImageNamed:@"cloud.bolt"];
    } else if ([descriptor isEqual:@"13d"]) {
        return [UIImage systemImageNamed:@"cloud.snow"];
    } else if ([descriptor isEqual:@"50d"]) {
